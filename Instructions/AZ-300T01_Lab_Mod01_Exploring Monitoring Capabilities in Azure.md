...
1. In the Azure portal, navigate to the blade representing the newly deployed Azure VM scale set and, from there, switch to the **Metrics** blade, under the **Monitoring** heading.

1. On the **Metrics** blade, use the filter to display **Percentage CPU** metric with an aggregation value of **Avg** of the VM scale set resource you provisioned in the previous exercise of this lab.
...
1. On the **Email/SMS/Push/Voice** blade, set an email address, a mobile phone number, or a phone number that you want to use to receive alerts generated by this rule, and then click **OK**.
...
1. In the **Condition** section, click **Add condition**, select the **Percentage CPU** metric, leave the dimension settings and alert logic threshold with their default values, set the Operator condition to **Greater than**, set the aggregation type to **Average**, set the threshold value to **60**, set the Aggregation granularity (period) to **1 minute**, set the frequency of evaluation to **Every 1 minute** and click **Done**.

1. In the **Actions** section, click **Select action group**, select previously created action group **az30001 action group** and click **Select**.
...
1. In the Azure portal, search for **Load balancers** and click the Load Balancers search result option. A load balancer should exist, representing the one created with the scale set you deployed from a template in a previous exercise of this lab.

1. From the Overview blade of the load balancer, identify the value of the **Public IP address** assigned to the front end of the load balancer associated with the VM scale set.

1. In a new browser tab or window, browse to **http://*Public IP address\*:9000** (where **_Public IP address_** is the IP address you identified in the previous step)

1. On the **Worker interface** page, click the **Start work** link.

1. From the Azure portal, use the **CPU (average)** chart on the VM scale set Overview blade to monitor changes to the CPU utilization.
   ...
   > **Note**: You should receive an alert regarding increased CPU utilization within a few minutes.
